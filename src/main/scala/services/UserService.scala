...
class UserService(repo: UserRepository) extends Service[User](repo) {
  ...
  def createUser(username: String, name: String): User = findUserByUsername(username) match {
    case Some(user) => {
  ...
  def updateNameForUser(newName: String, user: User) = repo.save(user.copy(name = newName))
